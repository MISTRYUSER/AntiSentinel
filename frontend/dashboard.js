...
async function selectSession(id) {
  ...
  const skill = data.skill_usage;
  const skillBlock = `<div class="section"><h3>SKILL</h3><div class="kv"><span>Skill</span><b>${esc(skill?.selected_skill_id || '—')}</b></div><div class="kv"><span>Version</span><b>${esc(skill?.selected_skill_version || '—')}</b></div><div class="kv"><span>Release</span><b>${esc((skill?.release_id || '—').slice(0, 12))}</b></div><div class="kv"><span>References</span><b>${Object.keys(skill?.reference_hashes || {}).length}</b></div></div>`;
  document.querySelector('#detail').innerHTML = `<div class="section"><h3>IDENTITY</h3><div class="kv"><span>Session</span><b>${esc(data.session_id.slice(0, 12))}</b></div><div class="kv"><span>Trace</span><b>${esc(data.trace_id || '—')}</b></div><div class="kv"><span>Status</span><b>${esc(data.status)}</b></div></div>${skillBlock}<div class="section"><h3>TOKEN USAGE</h3><div class="kv"><span>Input</span><b>${data.token_usage.input_tokens}</b></div><div class="kv"><span>Output</span><b>${data.token_usage.output_tokens}</b></div><div class="kv"><span>Cached input</span><b>${data.token_usage.cached_input_tokens}</b></div></div>${renderTraceGroup('CONTEXT BUILD', groups.context)}${renderTraceGroup('MEMORY RECALL / WRITE', groups.memory)}${renderTraceGroup('RAG RETRIEVAL', groups.rag)}${renderTraceGroup('MODEL COMPLETE', groups.model)}${renderTraceGroup('TOOL CALL / ATTEMPT', groups.tools)}${renderTraceGroup('WORKER', groups.worker)}<div class="section"><h3>MEMORY</h3><div class="kv"><span>Events</span><b>${data.event_count}</b></div><div class="kv"><span>Evidence refs</span><b>${data.evidence_ref_count}</b></div></div>`;
}
