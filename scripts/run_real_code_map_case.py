...
def main(argv=None):
    ...
    report = {
        ...
        "output_chunks": len(built.rows.chunks),
        "persisted_nodes": store.database.query("SELECT count(*) n FROM code_map_nodes WHERE snapshot_id=?", (built.snapshot.snapshot_id,))[0]["n"] if ready else 0,
        "persisted_edges": store.database.query("SELECT count(*) n FROM code_map_edges WHERE snapshot_id=?", (built.snapshot.snapshot_id,))[0]["n"] if ready else 0,
        "persisted_chunks": store.database.query("SELECT count(*) n FROM code_map_chunks WHERE snapshot_id=?", (built.snapshot.snapshot_id,))[0]["n"] if ready else 0,
