...
def build_code_map_tools(query, scope_provider: Callable[[dict], object], source_evidence_service=None) -> tuple[ToolDefinition, ...]:
    ...
    return (
        ToolDefinition("code_map.get_snapshot", "Read one published code-map snapshot metadata (no source body).", {**common, "properties": {"repository_id": {"type": "string"}, "requested_commit": {"type": "string"}}, "required": ["repository_id", "requested_commit"]}, get_snapshot),
        ToolDefinition("code_map.find_symbols", "Locate symbols by exact qualified_name; returns path/lines/chunk_id pointers.", {**common, "properties": {"repository_id": {"type": "string"}, "snapshot_id": {"type": "string"}, "qualified_name": {"type": "string"}, "limit": {"type": "integer"}}, "required": ["repository_id", "snapshot_id", "qualified_name"]}, find_symbols),
        ToolDefinition("code_map.get_node", "Read one code-map node pointer (kind/path/lines/chunk_id).", {**common, "properties": {"repository_id": {"type": "string"}, "snapshot_id": {"type": "string"}, "node_id": {"type": "string"}}, "required": ["repository_id", "snapshot_id", "node_id"]}, get_node),
        ToolDefinition("code_map.get_neighbors", "Traverse bounded code-map neighbors (graph pointers only).", {**common, "properties": {"repository_id": {"type": "string"}, "snapshot_id": {"type": "string"}, "node_id": {"type": "string"}, "direction": {"type": "string"}, "relations": {"type": "array"}, "depth": {"type": "integer"}, "node_budget": {"type": "integer"}}, "required": ["repository_id", "snapshot_id", "node_id"]}, get_neighbors),
        ToolDefinition("code_map.read_source", "Read one hash-verified source chunk body. Prefer locate via find_symbols/get_node first; body is ephemeral in context—re-read if needed later.", {**common, "properties": {"repository_id": {"type": "string"}, "snapshot_id": {"type": "string"}, "chunk_id": {"type": "string"}, "node_id": {"type": "string"}}, "required": ["repository_id", "snapshot_id"]}, read_source),
    )
