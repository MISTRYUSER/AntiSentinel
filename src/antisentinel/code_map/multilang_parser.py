...
class MultiLanguageParser:
    extensions = {'.go': 'go', '.ts': 'typescript', '.tsx': 'typescript', '.java': 'java'}
    ...
    def parse_file(self, path: str, data: bytes, node_scope: str) -> ParsedFile:
        ...
        pats = ([(r'^\s*type\s+(\w+)\s+struct\b','class'),(r'^\s*func\s+(?:\([^)]*\)\s+)?(\w+)\s*\(','function')] if lang=='go' else ([(r'^\s*(?:public|private|protected)?\s*(?:abstract\s+)?class\s+(\w+)','class'),(r'^\s*(?:public|private|protected|static|final|synchronized|native|abstract|\s)+[\w<>\[\], ?]+\s+(\w+)\s*\([^;]*\)\s*\{','function')] if lang=='java' else [(r'^\s*(?:export\s+)?(?:abstract\s+)?class\s+(\w+)','class'),(r'^\s*(?:export\s+)?(?:async\s+)?function\s+(\w+)','function'),(r'^\s*(?:export\s+)?(?:const|let)\s+(\w+)\s*=\s*(?:async\s*)?\(','function')]))
        nodes=[]
